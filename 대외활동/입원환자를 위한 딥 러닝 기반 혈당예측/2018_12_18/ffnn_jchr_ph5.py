...
for j in range(len(testData_in_list)):
    ...
    print("Patient: "+str(gitTestFileName[j])+", RMSE: "+str(rmse))
    
    
    #실제-예측 데이터를 텍스트 파일로 저장 (나중에 엑셀같은걸로 차트만들때 편하라고)
    realFp = open("chartData/FFNN/jchr/PH"+str(PH)+"/"+gitTestFileName[j]+"-real.txt", "w")
    predictFp = open("chartData/FFNN/jchr/PH"+str(PH)+"/"+gitTestFileName[j]+"-predict.txt", "w")

    #고/저혈당 실제-예측 횟수 데이터 텍스트 파일로 저장
    predictedHyperFp = open("chartData/FFNN/jchr/PH"+str(PH)+"/"+gitTestFileName[j]+"-predictedHyperList.txt", "w")
    desiredHyperFp = open("chartData/FFNN/jchr/PH"+str(PH)+"/"+gitTestFileName[j]+"-desiredHyperList.txt", "w")
    
    predictedHypoFp = open("chartData/FFNN/jchr/PH"+str(PH)+"/"+gitTestFileName[j]+"-predictedHypoList.txt", "w")
    desiredHypoFp = open("chartData/FFNN/jchr/PH"+str(PH)+"/"+gitTestFileName[j]+"-desiredHypoList.txt", "w")

    falseHighFp = open("chartData/FFNN/jchr/PH"+str(PH)+"/"+gitTestFileName[j]+"-falseHigh.txt", "w")
    falseLowFp =open("chartData/FFNN/jchr/PH"+str(PH)+"/"+gitTestFileName[j]+"-falseLow.txt", "w")
...
for xx in rmseList:
    ...
    print("Patient: "+str(gitTestFileName[count])+", standardDeviation: "+str(standardDeviation)+", RMSE: "+str(xx))
    count+=1

print("patient")
for i in range(len(rmseList)):
    print(str(gitTestFileName[i]))    
