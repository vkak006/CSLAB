...
def readData(filePath):
    ...
    while True:
        ...
        newPointy.append(float(df[4 + (PH)]))  # 데이터프레임 그 다음(10번째)을 newPointy 리스트에 삽입
        ...
        if (math.isnan(df[4 + (PH)])):  # 만약 8번째 데이터프레임이 NaN(Not a Number) 라면 반복 중지
            break
...
# input place holders
X = tf.placeholder(tf.float32, [None, 5, 1], name="X")
