...
x = LSTM(20, activation="linear", return_sequences=True)(inputA)
x = Bidirectional(LSTM(10,activation="linear"))(x)
...
for i in range(len(train_x_data)):
    model.fit([train_x_data[i],train_age_data[i]],train_y_data[i],epochs=50,batch_size=10)

model.save('(PH'+ str(PH) +')'+ date +'.h5')

RMSE = 0

for p in range(len(train_x_data)):
    #y_pred = model.predict(train_x_data[p])
    y_pred = model.predict([train_x_data[p],train_age_data[p]])
    MSE = 0
    size = 0
