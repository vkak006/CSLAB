...
for loop in range(len(trainData_in_list)):
    ...
    print(str(loop/(len(gitTrainFileName) + len(schTrainFileName))*100)+"%")
...
for j in range(len(testData_in_list)):
    ...
    print("Patient: "+str(totalTestFileName[j])+", RMSE: "+str(rmse))
    
    
    #실제-예측 데이터를 텍스트 파일로 저장 (나중에 엑셀같은걸로 차트만들때 편하라고)
    realFp = open("chartData/FFNN/jchr+sch/PH"+str(PH)+"/"+totalTestFileName[j]+"-real.txt", "w")
    predictFp = open("chartData/FFNN/jchr+sch/PH"+str(PH)+"/"+totalTestFileName[j]+"-predict.txt", "w")

    #고/저혈당 실제-예측 횟수 데이터 텍스트 파일로 저장
    predictedHyperFp = open("chartData/FFNN/jchr+sch/PH"+str(PH)+"/"+totalTestFileName[j]+"-predictedHyperList.txt", "w")
    desiredHyperFp = open("chartData/FFNN/jchr+sch/PH"+str(PH)+"/"+totalTestFileName[j]+"-desiredHyperList.txt", "w")
    
    predictedHypoFp = open("chartData/FFNN/jchr+sch/PH"+str(PH)+"/"+totalTestFileName[j]+"-predictedHypoList.txt", "w")
    desiredHypoFp = open("chartData/FFNN/jchr+sch/PH"+str(PH)+"/"+totalTestFileName[j]+"-desiredHypoList.txt", "w")

    falseHighFp = open("chartData/FFNN/jchr+sch/PH"+str(PH)+"/"+totalTestFileName[j]+"-falseHigh.txt", "w")
    falseLowFp =open("chartData/FFNN/jchr+sch/PH"+str(PH)+"/"+totalTestFileName[j]+"-falseLow.txt", "w")
...
for xx in rmseList:
    ...
    print("Patient: "+str(totalTestFileName[count])+", standardDeviation: "+str(standardDeviation)+", RMSE: "+str(xx))
    count+=1

print("patient")
for i in range(len(rmseList)):
    print(str(totalTestFileName[i]))    
