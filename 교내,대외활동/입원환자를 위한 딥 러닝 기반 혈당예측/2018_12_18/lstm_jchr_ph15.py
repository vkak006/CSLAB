...
for j in range(len(testData_in_list)):
    ...
    fileName = gitTestFileName[j]
    
    #실제-예측 데이터를 텍스트 파일로 저장 (나중에 엑셀같은걸로 차트만들때 편하라고)
    realFp = open("chartData/LSTM/jchr/PH"+str(PH)+"/"+fileName+"-real.txt", "w")
    predictFp = open("chartData/LSTM/jchr/PH"+str(PH)+"/"+fileName+"-predict.txt", "w")

    #고/저혈당 실제-예측 횟수 데이터 텍스트 파일로 저장
    predictedHyperFp = open("chartData/LSTM/jchr/PH"+str(PH)+"/"+fileName+"-predictedHyperList.txt", "w")
    desiredHyperFp = open("chartData/LSTM/jchr/PH"+str(PH)+"/"+fileName+"-desiredHyperList.txt", "w")
    
    predictedHypoFp = open("chartData/LSTM/jchr/PH"+str(PH)+"/"+fileName+"-predictedHypoList.txt", "w")
    desiredHypoFp = open("chartData/LSTM/jchr/PH"+str(PH)+"/"+fileName+"-desiredHypoList.txt", "w")

    falseHighFp = open("chartData/LSTM/jchr/PH"+str(PH)+"/"+fileName+"-falseHigh.txt", "w")
    falseLowFp =open("chartData/LSTM/jchr/PH"+str(PH)+"/"+fileName+"-falseLow.txt", "w")
