...
client_data = [] #Gateway 정보 리스트.
device_data = [] #Device 정보 리스트
device = {} #Device_id : {센서정보,센서정보,센서정보}, Device_id
a = 1
...
for i in client_data:
    device_data = []
    sensors = client.search(index = 'test3', doc_type = 'sensors', body = {'query':{'match':{'device_id':i['device_id']}}})['hits']

    if sensors['total'] == 0:
        #Device 정보만 저장
        print(i['name'] + " - Not found sensors info")
        with open("deviceId/" + i['device_id'] + ".json",'w') as f:
            f.write(str(i))
    ...
    device[i['device_id']] = device_data
...
for i in client_data:
    ...
    if not os.path.isdir("sensors/" + i['device_id']):
        os.mkdir("sensors/" + i['device_id'])

    for j in device[i['device_id']]:
        with open("sensors/"+ i['device_id'] + "/" + str(j['resource_id']) + ".json", 'w') as s:
            s.write(str(j))



###### (3) items, sitemap 스크립트 작성 부
#센서가 추가되었을 때의 경우는 고려하지않았음. total값으로 유추하도록 코딩 수정
#item
for i in client_data:
    client_str = "String state_" + str(a) + " \"상태[%s]\"\n"
    client_str += "String gateway_id_" + str(a) + " \"설치 지역[%s]\"\n"
    client_str += "String uid_" + str(a) + " \"단말 고유번호[%s]\"\n"
    client_str += "String name_" + str(a) + " \"단말 명칭[%s]\"\n"
    client_str += "String date_" + str(a) + " \"단말 생성일자[%s]\"\n"
    client_str += "String user_id_" + str(a) + " \"사용자[%s]\"\n"
    client_str += "String gps_lat_" + str(a) + " \"설치위치(위도)[%s]\"\n"
    client_str += "String gps_lon_" + str(a) + " \"설치위치(경도)[%s]\""
         
    with open("/etc/openhab2/items/client_" + str(a) + ".items", 'w') as f:
        f.write(client_str)
            
    #sitemap_name = i['name'].replace(" ","_")
    #차후, snode -> gateway_id로 수정한다.
    ...
    if not os.path.isfile("/etc/openhab2/items/" + i['device_id'] + ".items"):
        for j in device[i['device_id']]:
            item_str = "String " + j['type'] + "_" + str(a) + " \"" + changeType(j['resource_id'])[0] + "[%s]\" "+changeType(j['resource_id'])[1]+"\n"
        ...
        with open("/etc/openhab2/items/" + i['device_id'] + ".items", 'w') as f:
            f.write(item_str)
                
        item_main = "String state_" + str(a) + " \"받은 데이터[%s]\" {mqtt=\"<[mosq:withTopic/#:state:default]\"}"
        sitemap_str += "\n}"

        with open("/etc/openhab2/items/" + i['device_id'] + ".items", 'a') as f:
            f.write(item_main)

        with open("/etc/openhab2/sitemaps/" + sitemap_name + ".sitemap", 'w') as f:
            f.write(sitemap_str)
        a = a + 1
...
for i in device:
    ...
    rwObject=rw.RuleWriter(["java.io.File", "java.io.FileReader","java.io.BufferedReader"])
    rwObject.ruleMain(i,device[i],num)
